...
namespace bs
{
	...
	public:
		 ...
		 */
		static SPtr<MemoryDataStream> Compress(const SPtr<DataStream>& input, std::function<void(float)> reportProgress = nullptr);

		/**
		 * Decompresses the data from the provided data stream and outputs the new stream with decompressed data. Accepts
		 * an optional callback to be triggered during the process to report progress in range [0, 1].
		 */
		static SPtr<MemoryDataStream> Decompress(const SPtr<DataStream>& input, std::function<void(float)> reportProgress = nullptr);
	};
