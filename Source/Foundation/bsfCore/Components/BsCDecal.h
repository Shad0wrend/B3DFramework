...
namespace bs
{
	...
	public:
		...
		BS_SCRIPT_EXPORT(ExportName(Size),pr:setter)
		void SetSize(const Vector2& size) { mInternal->SetSize(size); }

		/** @copydoc setSize */
		BS_SCRIPT_EXPORT(ExportName(Size),pr:getter)
		...
		BS_SCRIPT_EXPORT(ExportName(Layer),pr:setter)
		void SetLayer(u64 layer) { mInternal->SetLayer(layer); }

		/** @copydoc setLayer() */
		BS_SCRIPT_EXPORT(ExportName(Layer),pr:getter)
		u64 GetLayer() const { return mInternal->GetLayer(); }

		BS_SCRIPT_EXPORT(ExportName(LayerMask),pr:setter)
		/** @copydoc Decal::setLayerMask */
		void SetLayerMask(u32 mask) { mInternal->SetLayerMask(mask); }

		BS_SCRIPT_EXPORT(ExportName(LayerMask),pr:getter)
