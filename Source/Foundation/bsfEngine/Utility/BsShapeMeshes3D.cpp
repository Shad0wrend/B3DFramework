...
namespace bs
{
	...
	void ShapeMeshes3D::WireHemisphere(const Sphere& sphere, const SPtr<MeshData>& meshData, u32 vertexOffset,
		u32 indexOffset, u32 quality)
	{
		u32 requiredNumVertices, requiredNumIndices;
	...
	void ShapeMeshes3D::SolidSphere(const Sphere& sphere, const SPtr<MeshData>& meshData, u32 vertexOffset,
		u32 indexOffset, u32 quality)
	{
		const SPtr<VertexDataDesc>& desc = meshData->GetVertexDesc();
	...
	void ShapeMeshes3D::WireDisc(const Vector3& center, float radius, const Vector3& normal, const SPtr<MeshData>& meshData,
		u32 vertexOffset, u32 indexOffset, u32 quality)
	{
	...
	void ShapeMeshes3D::WireArc(const Vector3& center, float radius, const Vector3& normal, Degree startAngle, Degree amountAngle,
		const SPtr<MeshData>& meshData, u32 vertexOffset, u32 indexOffset, u32 quality)
	{
		u32* indexData = meshData->GetIndices32();
		u8* positionData = meshData->GetElementData(VES_POSITION);
	...
	void ShapeMeshes3D::SolidAaBox(const AABox& box, u8* outVertices, u8* outNormals, u8* outUVs,
	...
	void ShapeMeshes3D::SolidSphere(const Sphere& sphere, u8* outVertices, u8* outNormals, u8 *outUV,
	...
	void ShapeMeshes3D::SolidArc(const Vector3& center, float radius, const Vector3& normal, Degree startAngle,
		Degree amountAngle, u8* outVertices, u8* outNormals, u8* outUV, u32 vertexOffset, u32 vertexStride,
	...
	void ShapeMeshes3D::SolidCone(const Vector3& base, const Vector3& normal, float height, float radius, Vector2 scale,
		u8* outVertices, u8* outNormals, u8* outUV, u32 vertexOffset, u32 vertexStride, u32* outIndices,
		u32 indexOffset, u32 quality)
	{
		outVertices += vertexOffset * vertexStride;
		outIndices += indexOffset;
	...
	void ShapeMeshes3D::SolidQuad(const Rect3& area, u8* outVertices, u8* outNormals, u8* outUV,
		u32 vertexOffset, u32 vertexStride, u32* outIndices, u32 indexOffset)
	{
		outVertices += (vertexOffset * vertexStride);
	...
	void ShapeMeshes3D::PixelLine(const Vector3& a, const Vector3& b, u8* outVertices,
		u32 vertexOffset, u32 vertexStride, u32* outIndices, u32 indexOffset)
	{
		outVertices += (vertexOffset * vertexStride);
	...
	void ShapeMeshes3D::AntialiasedLine(const Vector3& a, const Vector3& b, const Vector3& up, float width, float borderWidth, const Color& color, u8* outVertices, u8* outColors,
		u32 vertexOffset, u32 vertexStride, u32* outIndices, u32 indexOffset)
	{
		Vector3 dir = b - a;
		dir.Normalize();
	...
	void ShapeMeshes3D::PixelSolidPolygon(const Vector<Vector3>& points, u8* outVertices,
		u32 vertexOffset, u32 vertexStride, u32* outIndices, u32 indexOffset)
	{
		outVertices += (vertexOffset * vertexStride);
	...
	void ShapeMeshes3D::PixelWirePolygon(const Vector<Vector3>& points, u8* outVertices,
		u32 vertexOffset, u32 vertexStride, u32* outIndices, u32 indexOffset)
	{
		i32 numPoints = (i32)points.size();
		u32 curVertOffset = vertexOffset;
	...
	void ShapeMeshes3D::AntialiasedPolygon(const Vector<Vector3>& points, const Vector3& up, float borderWidth, const Color& color, u8* outVertices, u8* outColors,
		u32 vertexOffset, u32 vertexStride, u32* outIndices, u32 indexOffset)
	{
	...
	u32 ShapeMeshes3D::SubdivideTriangleOnSphere(const Vector3& center, float radius, u32 numLevels,
	...
	void ShapeMeshes3D::GenerateArcVertices(const Vector3& center, const Vector3& up, float radius, Degree startAngle,
		Degree angleAmount, Vector2 scale, u32 numVertices, u8* outVertices, u32 vertexOffset, u32 vertexStride)
	{
	...
	void ShapeMeshes3D::GenerateTangents(u8* positions, u8* normals, u8* uv, u32* indices,
		u32 numVertices, u32 numIndices, u32 vertexOffset, u32 indexOffset, u32 vertexStride, u8* tangents)
	{
