...
namespace bs
{
	namespace ct
	{
		...
		public:
			 ...
			 */
			void Parse(ID3DBlob* microcode, GpuProgramType type, GpuParameterDescription& desc, Vector<VertexElement>* inputParams);
		...
		private:
			 ...
			 */
			void ParseBuffer(ID3D11ShaderReflectionConstantBuffer* bufferReflection, GpuParameterDescription& desc);

			/**	Parses the resource description structure and stores it in the provided GPU params description object. */
			void ParseResource(D3D11_SHADER_INPUT_BIND_DESC& resourceDesc, GpuProgramType type, GpuParameterDescription& desc);
			 ...
			 */
			void ParseVariable(D3D11_SHADER_TYPE_DESC& varTypeDesc, D3D11_SHADER_VARIABLE_DESC& varDesc, GpuParameterDescription& desc, GpuDataParameterBlockInformation& paramBlock);
