...
namespace b3d
{
	namespace ct
	{
		...
		public:
			 ...
			 */
			void Parse(ID3DBlob* microcode, GpuProgramType type, GpuProgramParameterDescription& desc, Vector<VertexElement>* inputParams);
		...
		private:
			 ...
			 */
			void ParseBuffer(ID3D11ShaderReflectionConstantBuffer* bufferReflection, GpuProgramParameterDescription& desc);

			/**	Parses the resource description structure and stores it in the provided GPU params description object. */
			void ParseResource(D3D11_SHADER_INPUT_BIND_DESC& resourceDesc, GpuProgramType type, GpuProgramParameterDescription& desc);
			 ...
			 */
			void ParseVariable(D3D11_SHADER_TYPE_DESC& varTypeDesc, D3D11_SHADER_VARIABLE_DESC& varDesc, GpuProgramParameterDescription& desc, GpuDataParameterBlockInformation& paramBlock);
