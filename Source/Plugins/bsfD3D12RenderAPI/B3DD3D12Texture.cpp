...
void D3D12Texture::WriteData(const PixelData& data, u32 face, u32 mipLevel, bool discardEntireBuffer)
{
	...
	SPtr<GpuQueue> queue = device.GetQueue(GQT_GRAPHICS, 0);
	SPtr<render::GpuCommandBuffer> transferCommandBuffer = queue->GetOrCreateTransferCommandBuffer();
...
void D3D12Texture::ReadData(PixelData& data, u32 face, u32 mipLevel)
{
	...
	queue->SubmitTransferCommandBuffer(true); // true = wait for completion
