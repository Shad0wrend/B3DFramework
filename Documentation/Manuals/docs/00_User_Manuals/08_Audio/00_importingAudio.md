...
Audio in b3d::f is represented in the form of an @b3d::AudioClip object. An audio clip is a resource, meaning it can be imported, saved and loaded as we described in the resource manuals.
...
When an audio clip is loaded for playback, you can choose how is its data accessed, in order to tweak required memory and/or CPU usage by setting the read mode though @b3d::AudioClipImportOptions::readMode.

Available read modes are part of the @b3d::AudioReadMode enumeration. They are:
...
## Bit depth
@b3d::AudioClipImportOptions::bitDepth controls how many bits will a single audio sample be stored in. Smaller values yield smaller memory footprint, but lesser quality, and the other way around.
...
## 2D/3D
Audio files can be played back in 2D or 3D mode. 2D audio is the familiar audio playback, like music. 3D audio will control playback volume, pitch and other properties based on the position and velocity of the listener compared to the audio source. This can be controlled by setting the @b3d::AudioClipImportOptions::is3D. Note this is on by default.
