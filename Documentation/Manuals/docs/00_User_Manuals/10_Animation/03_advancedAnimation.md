...
# Cross fade
Often your animation system might need to transition from playing one animation clip to another (e.g. moving from a walk to a run animation). In that case you can use @b3d::CAnimation::crossFade instead of **CAnimation::play()**. Cross fade will slowly fade in the new animation clip while fading out the previously playing one, over the specified time period.
...
# Sampling
Sometimes it is useful to sample just a single frame from an animation clip (e.g. "freezing" an enemy in place). For such purpose you can use @b3d::CAnimation::sample. Just provide the animation clip and the time to sample at.
...
To start a 1D blend operation call @b3d::CAnimation::blend1D. This method expects a @b3d::Blend1DInfo structure, containing a list of animations to blend between, and their positions. It also expects the weight value that determines which animations are blended together.
...
To start a 2D blend operation call @b3d::CAnimation::blend2D. This method expects a @b3d::Blend2DInfo structure, containing four animation clips to blend between, as well as a 2D weight determining how the animations are blended together.
...
To play an additive animation call @b3d::CAnimation::blendAdditive. This method expects the following parameters:
 - A clip to play
...
To stop an additive animation you can call @b3d::CAnimation::stop(). It accepts a layer index as a parameter and determines which clip to stop. Calling **CAnimation::stopAll()** will stop all clips, additive or otherwise.
...
Call @b3d::CAnimation::getState with an animation clip as the parameter to retrieve information about that clip. The method outputs an @b3d::AnimationClipState that can be used to query various information about the clip.
...
To set a clip state call @b3d::CAnimation::setState. If a particular clip is already playing its state will be updated, and if the clip isn't currently playing, playback will start according to the assigned state.
...
The state has the following relevant properties:
 ...
 - @b3d::AnimationClipState::wrapMode - Wrap mode same as **CAnimation::setWrapMode()** except it can be set for each clip individually.
