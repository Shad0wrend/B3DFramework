...
They are represented with the @b3d::SpriteTexture class and are a **Resource**, same as normal textures. 

They're created by calling @b3d::SpriteTexture::create. As a parameter it expects the source **Texture**, and an optional set of UV coordinates that map to a specific area on the texture. If no coordinates are provided the sprite texture maps to the entirety of the texture, acting the same as a normal texture.
...
Once created, you can get the actual width/height of the mapped area by calling @b3d::SpriteTexture::getWidth and @b3d::SpriteTexture::getHeight.
...
You can also always retrieve the underlying texture by calling @b3d::SpriteTexture::getTexture.
...
# Animation
Sprite textures also support sprite sheet grid based animation. To initialize the animation you need to populate the @b3d::SpriteSheetGridAnimation structure and pass it along to @b3d::SpriteTexture::setAnimation.
...
Finally you need to enable animation playback by passing one of the @b3d::SpriteAnimationPlayback values to @b3d::SpriteTexture::setAnimationPlayback.
