...
To create a GPU program call @b3d::ct::GpuProgram::create() with a @b3d::GPU_PROGRAM_DESC structure. The structure needs to have the following fields populated:
...
Once the GPU program has been created it is not guaranteed to be usable. The compilation of the provided source code could have failed, which you can check by calling @b3d::ct::GpuProgram::isCompiled(), and retrieve the error message by calling @b3d::ct::GpuProgram::getCompileErrorMessage(). 
...
To use a GPU program in a draw or dispatch call, you must first create a GPU pipeline object using the relevant GPU programs. There are two types of pipeline states:
 - @b3d::ct::GraphicsPipelineState - Supports vertex, hull, domain, geometry and fragment programs. At minimum requires a vertex program, while most pipelines will use vertex & fragment programs.
 - @b3d::ct::ComputePipelineState - Supports only the compute GPU program type.

To create a graphics pipeline you need to fill out the @b3d::ct::PIPELINE_STATE_DESC structure with references to relevant GPU programs, followed by a call to @b3d::ct::GraphicsPipelineState::create().
...
Compute pipeline states are simpler, accepting just a single compute GPU program as a parameter to their @b3d::ct::ComputePipelineState::create() method.
...
Once created pipelines can be bound for rendering through the globally accessible @b3d::ct::RenderAPI interface. This interface is the primary entry point in the low-level rendering API and it will be used for most low-level rendering operations, as we'll see throughout this set of manuals.

Call @b3d::ct::RenderAPI::setGraphicsPipeline or @b3d::ct::RenderAPI::setComputePipeline to bind a graphics or a compute pipeline state, respectively.
...
You can access information about GPU program parameters by calling @b3d::ct::GpuProgram::getParamDesc. This will return a @b3d::GpuParamDesc structure containing information about all GPU parameters used by that GPU program. This includes primitives (int, float, etc.), textures, samplers, buffers and parameter buffers (constant/uniform buffers in DirectX/OpenGL lingo). 

You generally don't need to use this information directly. It is instead automatically parsed when you create a GPU pipeline. Once you have a pipeline you can use it to create a @b3d::ct::GpuParams object that allows you to assign values to all parameters of a specific pipeline.
...
To create a **GpuParams** object call @b3d::ct::GpuParams::create with either a graphics or a compute pipeline state as a parameter.
...
Once created you can assign values to parameter by calling any of the following methods (depending on parameter type):
 - @b3d::ct::GpuParams::setTexture(GpuProgramType, const String&, const TextureType&, const TextureSurface&) - Assigns a read-only (sampled) texture to a GPU program.
 - @b3d::ct::GpuParams::setLoadStoreTexture(GpuProgramType, const String&, const TextureType&, const TextureSurface&) - Assign a load-store (writable) texture to a GPU program.
 - @b3d::ct::GpuParams::setBuffer(GpuProgramType, const String&, const BufferType&) - Assigns a buffer (either read-only or read-write) to a GPU program.
 - @b3d::ct::GpuParams::setSamplerState(GpuProgramType, const String&, const SamplerType&) - Assigns a sampler state that determines how is a sampled texture read by the shader.
 - @b3d::ct::GpuParams::setParam<T>(GpuProgramType, const String&, const T&) - Assigns a primitive type like *float*, *int*, **Vector3**, **Matrix4** or others. Supported primitive types are:
  - *float*
  - **Vector2**
  - **Vector3**
  - **Vector4**
...
If parameters are modified often you can instead use *parameter handles* for faster access. Use the following methods to retrieve a handle to the parameter:
 - @b3d::ct::GpuParams::getTextureParam - Outputs a @b3d::TGpuParamTexture<Core> handle that can be used for reading & writing the parameter value.
 - @b3d::ct::GpuParams::getLoadStoreTextureParam - Outputs a @b3d::TGpuParamLoadStoreTexture<Core> handle that can be used for reading & writing the parameter value.
 - @b3d::ct::GpuParams::getBufferParam - Outputs a @b3d::TGpuParamBuffer<Core> handle that can be used for reading & writing the parameter value.
 - @b3d::ct::GpuParams::getSamplerStateParam - Outputs a @b3d::TGpuParamSampState<Core> handle that can be used for reading & writing the parameter value.
 - @b3d::ct::GpuParams::getParam<T> - Outputs a @b3d::TGpuDataParam<Core, T> handle that can be used for reading & writing the parameter value.
...
Before we can assign primitive parameters to **GpuParams** we must first allocate their parameter blocks (in the examples above we skipped this step). Parameter blocks are represented by @b3d::ct::GpuParamBlockBuffer. They are essentially just blocks of memory and only require a size in bytes in their call to @b3d::ct::GpuParamBlockBuffer::create().
...
Finally you can bind the parameter block to **GpuParams** by calling @b3d::ct::GpuParams::setParamBlockBuffer with the GPU program type, parameter block variable name, and the parameter block buffer.
...
> You can also write to **GpuParamBlockBuffer** directly by calling its @b3d::ct::GpuParamBlockBuffer::write method. In this case you must be careful to respect the layout of the variables in the buffer as expected by the render backend. This layout can be determined by examining the other entries in **GpuParamDesc** structure.

## Binding GPU params
Once **GpuParams** has been created and populated with necessary data, you can bind it to the GPU by calling @b3d::ct::RenderAPI::setGpuParams. 
...
# Vertex input
Vertex GPU programs provide information about their inputs in the form of a **VertexDeclaration**. This is the same structure that we used for describing per-vertex components while creating a mesh. Per-vertex input declaration can be retrieved from a GPU program by calling @b3d::ct::GpuProgram::getInputDeclaration.
